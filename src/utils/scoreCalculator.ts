...
const CHARACTER_WEIGHTS: Record<string, Record<string, number>> = {
  // カルロッタ（凝縮ダメージ主体）
'カルロッタ': {
    // メインオプション (image_4bb1fe.png基準: スコア15 ÷ 30% = 0.5)
    '凝縮ダメージアップ': 0.5,
    'クリティカル': 0.5,
    'クリティカルダメージ': 0.5,
    '攻撃力(%)': 0.5,

    // サブオプション (image_4bb1fe.pngの個別スコアから逆算)
    'sub_クリティカル': 2.0,               // 9.9% * 2.0 = 19.8
    'sub_クリティカルダメージ': 1.0,       // 15.0% * 1.0 = 15
    'sub_攻撃力(%)': 1.25,                // 9.4% * 1.25 = 11.75
    'sub_共鳴スキルダメージアップ': 1.0,   // 7.1% * 1.0 = 7.1
    'sub_共鳴効率': 0.4,                   // 星3評価に基づき既存値を維持
    'sub_共鳴解放ダメージアップ': 0.0,     // 評価対象外 (スコア0)
    'sub_通常攻撃ダメージアップ': 0.0,     // 星1評価のため0.0
    'sub_攻撃力(実数値)': 0.0              // 評価対象外 (スコア-)
},
};
